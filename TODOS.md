...
- [x] Restore parallel grammar preload (serialized in `edb2e64`); `preload_grammars`
      now fans `build_grammar` across a bounded `core:thread.Pool`.
  - [ ] Add tests: pool fan-out correctness + `-sanitize:thread` for race-freedom.
  - [ ] Concurrent-read path: an `RW_Mutex` on `grammar()` only helps a future
        multi-threaded render, not the load side. Revisit if render goes parallel.
